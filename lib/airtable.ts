 ...
 *
 * Airtable caps responses at 100 records and rate-limits to 5 requests/second
 * per base. Because this runs inside `use cache`, a full crawl happens only on
 * a cache miss rather than per visitor.
 */
...
export function attachments(
  ...
  return value.filter(
    (v): v is Attachment => v != null && typeof v === "object" && "url" in v,
  );
}
